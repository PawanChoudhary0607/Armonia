...
class AudioNotifier extends Notifier<AudioState> {
 ...
 case ProcessingState.completed:
  ...
  Future<void> playSong(Song song) async {
    ...
    // ── PHASE 4B — PLAYBACK DIAGNOSTICS ─────────────────────────────────
    // Pure measurement only — no behavior changes. Logs how long each of
    // the three major playback steps takes so slow steps can be identified
    ...
    try {
      final Stopwatch extractTimer = Stopwatch()..start();
      final String url = await _extractor.getAudioStreamUrl(song.videoId);
      extractTimer.stop();
      debugPrint(
        '[AudioProvider][TIMING] getAudioStreamUrl: '
        '${extractTimer.elapsedMilliseconds}ms (videoId=${song.videoId})',
      );
      ...
      final Stopwatch setSourceTimer = Stopwatch()..start();
      try {
        ...
        debugPrint(
          '[AudioProvider][TIMING] setAudioSource: '
          '${setSourceTimer.elapsedMilliseconds}ms (videoId=${song.videoId})',
        );
      } catch (e, st) {
        setSourceTimer.stop();
        ...
        debugPrint(
          '[AudioProvider][TIMING] setAudioSource: '
          '${setSourceTimer.elapsedMilliseconds}ms (FAILED, videoId=${song.videoId})',
        );
        rethrow;
      }
      ...
      final Stopwatch playTimer = Stopwatch()..start();
      try {
        await _player.play();
        playTimer.stop();
        debugPrint('[AudioProvider] play success');
        debugPrint(
          '[AudioProvider][TIMING] play(): '
          '${playTimer.elapsedMilliseconds}ms (videoId=${song.videoId})',
        );
      } catch (e, st) {
        playTimer.stop();
        ...
        debugPrint(
          '[AudioProvider][TIMING] play(): '
          '${playTimer.elapsedMilliseconds}ms (FAILED, videoId=${song.videoId})',
        );
        rethrow;
      }
      ...
      totalTimer.stop();
      debugPrint(
        '[AudioProvider][TIMING] TOTAL playSong→audible: '
        '${totalTimer.elapsedMilliseconds}ms (videoId=${song.videoId}, '
        'title="${song.title}")',
      );

      // Record this song in Recently Played now that playback has
      // definitively started for this generation. This is a side effect
      // only — it does not influence playback state above.
      if (generation == _loadGeneration) {
        try {
          ref.read(recentlyPlayedProvider.notifier).add(song);
        } catch (e) {
          debugPrint('[AudioProvider] recentlyPlayed.add FAILED (ignored): $e');
        }
      }
