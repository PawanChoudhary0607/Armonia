...
class StreamExtractor {
  ...
  /// YouTube API clients tried in order, most-reliable-first.
  ///
  /// Selection rationale (derived from `youtube_explode_dart` 2.5.3 source
  /// comments and the library's own default client precedence):
  ///
  /// 1. [YoutubeApiClient.androidVr]   — "Provides high quality videos."
  ///    No signature deciphering required. Stable on Android. High-bitrate
  ///    audio-only streams. Best overall first choice for a music app.
  ///
  /// 2. [YoutubeApiClient.androidMusic] — "Works only for music."
  ///    Targets the YouTube Music endpoint directly. No sig deciphering.
  ///    Ideal fallback when androidVr stalls; purpose-built for this use case.
  ///
  /// 3. [YoutubeApiClient.ios]          — "Has limited streams but doesn't
  ///    require signature deciphering." The library's own proven default.
  ///    Reliable across regions; kept as the third attempt.
  ///
  /// 4. [YoutubeApiClient.tv]           — The library's own automatic
  ///    last-resort (it retries with `tv` internally when all user-supplied
  ///    clients fail). Handles restricted/age-gated videos. Requires
  ///    signature deciphering but rarely times out.
  ///
  /// 5. [YoutubeApiClient.safari]       — WEB client impersonating Safari on
  ///    macOS. Performs worst on Android (no native browser context). Kept
  ///    only as the final safety net.
  ///
  /// Clients NOT included:
  /// - [YoutubeApiClient.android]     — Source comment: "seems less reliable
  ///   than ios." Superseded by androidVr.
  /// - [YoutubeApiClient.mweb]        — "Sometimes includes low quality
  ///   streams (eg. 144p12)." Unacceptable for audio.
  /// - [YoutubeApiClient.mediaConnect] — Undocumented; no evidence of
  ///   reliability for audio extraction.
  static final List<YoutubeApiClient> _clientsInOrder = [
    YoutubeApiClient.androidVr,    // 1st — high quality, no sig decipher
    YoutubeApiClient.androidMusic, // 2nd — music-specific endpoint
    YoutubeApiClient.ios,          // 3rd — library default, no sig decipher
    YoutubeApiClient.tv,           // 4th — handles restricted videos
    YoutubeApiClient.safari,       // 5th — last resort only
  ];
